...
async function run() {
    try {
        ...
        // Find a BP that might have it checked
        const res = await sapRequest(`${baseUrl}/BusinessPartners?$top=1&$filter=SubjectToWithholdingTax eq 'tYES' or SubjectToWithholdingTax eq 'boYES'`, { headers });
