...
async function main() {
    ...
    const bpRes = await sapRequest(`${baseUrl}/BusinessPartners?$filter=CardType eq 'cSupplier'&$select=CardCode,CardName,GroupCode&$top=20`, { headers });
    console.log("Recent Suppliers CardCodes:");
    if (bpRes.data && bpRes.data.value) {
        bpRes.data.value.forEach(bp => console.log(bp.CardCode, bp.CardName));
    }
}
main();
