...
async function main() {
    ...
    // Search for 23359505 in ChartOfAccounts
    const query = `ChartOfAccounts?$filter=FormatCode eq '23359505' or Code eq '23359505'&$select=Code,FormatCode,Name,AccountType`;
    const res = await sapRequest(`${baseUrl}/${query}`, { headers });
    console.log("Account Details:", JSON.stringify(res.data, null, 2));
}
main();
